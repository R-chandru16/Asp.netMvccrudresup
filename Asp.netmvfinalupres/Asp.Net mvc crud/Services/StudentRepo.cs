...
namespace Asp.net_student.Services
{
    public class StudentRepo : IRepo<Student>
    {
        ...
        //public List<StudentRepo> SearchStudents(string search)
        //{

            
        //    return _context.Students
        //      .Where(s => s.Name.Contains(search) || s.Email.Contains(search) || s.Age == age)
        //      .ToList();
        //}
        ...
        List<Student> IRepo<Student>.SearchStudents(string search)
        {
            int age;
            bool success = Int32.TryParse(search, out age);
            if (!success)
            ...
            return _context.Students
            .Where(s => s.Name.StartsWith(search)|(s.Gender == search || search == null )||
                         s.country.Contains(search) || s.Age == age)
            .ToList();
